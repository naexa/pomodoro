...
## 履歴ページ（/history）新規作成（完了）

### 目的
「履歴を見る」ボタンから遷移する `/history` ページを新規作成し、過去全ての完了タスク履歴を日付別に表示する。

### 背景
- `TaskList.tsx` の完了タスクアコーディオン内に「履歴を見る」ボタンがあったが、リンク先が存在しなかった
- 過去全ての完了タスクを日付指定やページネーションで閲覧したいという要望

### 実装内容

#### Step 1: HistoryPage.tsx 新規作成
- [x] `fetchTaskHistory()` で全履歴取得
- [x] 日付降順でソート（新しい順）
- [x] 日付範囲フィルタ（開始日・終了日）
- [x] ページネーション（1ページ10日分）
- [x] カテゴリバッジ・完了時刻表示

#### Step 2: main.tsx にルート追加
- [x] `/history` ルート追加

#### Step 3: TaskList.tsx のリンク修正
- [x] `<a href="#/history">` → `<Link to="/history">` に変更

### 変更対象ファイル

| ファイル | 操作 | 内容 |
|----------|------|------|
| `app/src/pages/HistoryPage.tsx` | 新規 | 履歴ページコンポーネント |
| `app/src/main.tsx` | 編集 | `/history` ルート追加 |
| `app/src/components/Tasks/TaskList.tsx` | 編集 | リンクを `<Link>` に変更 |

### 完了条件

- [x] `/history` ページが表示される
- [x] 過去全ての完了タスクが日付降順で表示される
- [x] 日付範囲フィルタが動作する
- [x] ページネーションが動作する
- [x] YouTubeミニプレイヤーが正常動作する（Portal方式で自動対応）
- [x] 「履歴を見る」ボタンから正しく遷移できる
- [x] ビルド成功
